...
# Pre-processing script for Experiment 2 of the control adjustment costs project. 
...
thresh_minIntervals_performed = 96  # Total intervals = 120, threshold is 80 % 
...
df.trial.raw1 = read.csv('../data/RawData/2021-12-18_4.0/trialdata_debug_2021-12-18.csv')
df.trial.raw2 = read.csv('../data/RawData/2021-12-18_4.0/trialdata_speedaccuracy_2021-12-18.csv')
...
data.prolific = read.csv("../data/RawData/2021-12-18_4.0/prolific_export_6197e082183b06f1f3f5cfe1.csv")

# Load in the attention quiz data
df.quiz1 = read.csv('../data/RawData/2021-12-18_4.0/questiondata_debug_2021-12-18.csv') %>% 
  separate(col = uniqueid, sep = ":", c("SubID","Version"), remove = FALSE) %>%
  filter(uniqueid %in% df.trial$uniqueid)

df.quiz2 = read.csv('../data/RawData/2021-12-18_4.0/questiondata_speedaccuracy_2021-12-18.csv') %>% 
  separate(col = uniqueid, sep = ":", c("SubID","Version"), remove = FALSE) %>%
  filter(uniqueid %in% df.trial$uniqueid)
...
#### Exclude subjects based on accuracy and remove the short RTs ####
...
write.csv(x = df.trial, file = paste0('../data/ProcessedData/','CleanData', 'SpeedAccuracySpeedAcc','_nSubs_',length(unique(df.trial$SubID)),'.csv'), row.names = FALSE)
write.csv(x = df.trial, file = paste0('../brms/Cluster/data/', 'SpeedAccuracySpeedAcc','_nSubs_',length(unique(df.trial$SubID)),'.csv'), row.names = FALSE)
...
df.DDM = df.trial %>%
  ...
  filter(!is.na(previntervalType))%>%
  dplyr::select(SubID,trialNum,hit,rt,intervalType,congruence,previntervalType,scaledRunningTime) %>%
  mutate(rt=rt/1000,
         response=hit,
         congruency=ifelse(congruence==1,"congruent","incongruent")) %>%
...
check=df.DDM%>%
  group_by(subj_idx, intervalType,previntervalType) %>%
  summarize(Freq=n(hit))
...
write.csv(preproSummary, file = "PreprocessingSummary_Experiment2.csv")
