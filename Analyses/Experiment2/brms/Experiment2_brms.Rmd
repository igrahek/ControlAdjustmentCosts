...
Experiment: Control adjustment costs Experiment 2 (Speed vs. Accuracy vs. Speed-Accuracy conditions)
Code written by: Ivan Grahek (2023)
Description: Code for the analysis of behavioral data for Experiment 2 of the Control Adjustment costs project.  
...
model = readRDS("Cluster/output/model.RT_prevInt.rds")
...
# Default plot
p = conditional_effects(model, effects = "intervalType:previntervalType")

# Convert to a data frame
p = as.data.frame(p$`intervalType:previntervalType`)
# 


p$intervalType = factor(p$intervalType, levels = c("Speed", "Speed_Accuracy", "Accuracy"))
p$previntervalType = factor(p$previntervalType, levels = c("Speed", "Speed_Accuracy", "Accuracy"))

levels(p$intervalType) = c("Speed","Speed & Accuracy" , "Accuracy")
levels(p$previntervalType) = c("Speed","Speed & Accuracy" , "Accuracy")


# Plot
p = ggplot(p,aes(x=intervalType,y=estimate__,color=previntervalType)) + 
  geom_pointrange(aes(ymin=lower__, ymax=upper__),position=position_dodge(0.4),size=2, fatten = 2) + 
  guides(color = guide_legend(override.aes=list(shape = 20)))+
  ylab("Reaction times (ms)")+
  xlab("Performance goal")+
  scale_x_discrete(limits = c("Speed","Speed & Accuracy","Accuracy"), expand=c(0.2, 0.2))+
  # scale_color_manual(values = c("Speed" = Dark, "Speed & Accuracy" = Medium,"Accuracy" = Light)) +
  scale_y_continuous(limits = c(500,800))+
  
  theme(axis.line = element_line(colour = "black"),plot.margin=grid::unit(c(5,85,5,5), "mm"),
        axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 0)),
...
model = readRDS("Cluster/output/model.Acc_prevInt.rds")
...
# pdf(file="../../../Manuscript/figures/Figure_Fig3_a_behavior.pdf",height=4,width=16)
# 
# p = cowplot::plot_grid(Fig3.RT,
#                        Fig3.Acc,
#                        nrow = 1)
# p
# dev.off()
# 
# pdf(file="../../../Manuscript/figures/Figure_Fig3_b_behavior.pdf",height=4,width=22)
# 
# p = cowplot::plot_grid(Fig3.RT_prev,
#                        Fig3.Acc_prev,
#                        nrow = 1)
# p
# dev.off()

pdf(file="../../../Manuscript/figures/Figure_3_behavior.pdf",height=6,width=19)
